...
\tikzstyle{io} = [trapezium, trapezium left angle=70, trapezium right angle=110, text width=3.75cm, minimum height=0.5cm, text centered, draw=black]
\tikzstyle{process} = [rectangle, minimum width=2.5cm, text width=4cm, minimum height=0.5cm, text centered, draw=black]
...
\usepackage{pgfplots}
\pgfplotsset{compat=newest, samples=65} %Set this value to 65 for the final version
...
\begin{figure}
	...
	\begin{tikzpicture}[node distance = 3mm, auto]
		...
		\node (Calculate01a) [process, below = of Input01] {CALCULATE $a=\frac{0.99}{n}$};
		...
		\node (Decision01) [decision, below = of Output03, text width=2.5cm] {CONTINUE\\EXPERIMENTS?};
		...
		\draw [arrow] ($ (Decision01.east) $)node[anchor=north west]{Yes}  -- ++(0.6,0.00) |- ($ (Input03.north) + (0mm,2mm) $);
