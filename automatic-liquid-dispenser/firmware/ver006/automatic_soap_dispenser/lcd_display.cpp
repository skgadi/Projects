...
void LCD_DISPLAY::displayProg(unsigned int total) {
  if (enableDisplay) {
    ...
    lcd.print(textForDisplay[7]);
      ...
      drawVerticalLine(8);
      lcd.setCursor(9,0);
      lcd.print(textForDisplay[0] + commaForInt(total,10,0));
      lcd.setCursor(9,1);
...
void LCD_DISPLAY::displayViewSales(int chemNo, unsigned long sales, unsigned long cons) {
  if (enableDisplay) {
    ...
    lcd.setCursor(0,1);
    lcd.print(textForDisplay[15] + textForDisplay[1] + textForDisplay[11] + commaForInt(sales, 6, 0));
    lcd.setCursor(0,2);
    lcd.print(textForDisplay[16] + textForDisplay[1] + textForDisplay[11] + commaForInt(cons, 6, 2));
    lcd.setCursor(0,3);
    lcd.print(3+textForDisplay[1]+textForDisplay[7]);
    printLogo(3, 17);
    enableDisplay = false;
  }  
}
