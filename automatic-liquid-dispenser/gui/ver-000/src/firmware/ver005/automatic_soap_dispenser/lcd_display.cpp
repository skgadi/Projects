...
void LCD_DISPLAY::displayViewSales(int chemNo, unsigned long sales, unsigned long cons) {
  if (enableDisplay) {
    ...
    lcd.setCursor(0,1);
    lcd.print(textForDisplay[15] + textForDisplay[1] + textForDisplay[11] + commaForInt(sales, 7, 0));
    lcd.setCursor(0,2);
    lcd.print(textForDisplay[16] + textForDisplay[1] + textForDisplay[11] + commaForInt(cons, 9, 2));
    lcd.setCursor(0,3);
