...
void ReadTimeFromBuffer (char *Output) {
    /*char buffer[EUSART1_RX_BUFFER_SIZE];
    *buffer = *eusart1RxBuffer;*/
    const char StartCode[7]="$GPRMC,";
    char Pos=0x00;
    char TempChar = 0x00;
...
BOOL ValidateDateTime (char *Date, char *Time) {
    ...
    UINT8 Item = (Date[0]-0x30)*10 + (Date[1]-0x30);
    if (Item>31) return false;
    Item = (Date[2]-0x30)*10 + (Date[3]-0x30);
    if (Item>12) return false;
    Item = (Date[4]-0x30)*10 + (Date[5]-0x30);
    if (Item>99) return false;

    Item = (Time[0]-0x30)*10 + (Time[1]-0x30);
    if (Item>23) return false;
    Item = (Time[2]-0x30)*10 + (Time[3]-0x30);
    if (Item>59) return false;
