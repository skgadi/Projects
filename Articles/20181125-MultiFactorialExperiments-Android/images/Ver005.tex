...
\begin{figure}
	...
	\begin{tikzpicture}[node distance = 5mm, auto]
		...
		\node (Calculate02) [process, below = of Input02] {CALCULATE\\$x_i^I$ AND $x_i^M$\\USING (14) AND (15)};
		\node (Input03) [io, below = of Calculate02, text width=3.25cm] {INPUT $x_i$};
		\node (Calculate03) [process, below = of Input03] {RESET $\Sigma_z=0$};
		\node (Calculate04) [process, below = of Calculate03] {CALCULATE $z_i$\\ USING (13)};
...
\begin{figure}
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Surface plot of $F$ with the constants given in Section~$\ref{Sec:Application}$ superimposed with the RSM results.}
...
\begin{figure}
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Contour plot of $F$ with $K_R=0$ and the other constants given in Section~$\ref{Sec:Application}$ superimposed with the RSM results.}
...
\begin{figure}
	...
	\end{axis}
	\end{tikzpicture}
	\caption{Surface plot of $F$ with $K_R=0$ and the other constants given in Section~$\ref{Sec:Application}$ superimposed with the RSM results.}
